...
public class MongoHandlerRaw {
    ...
    private static final MongoHandlerRaw INSTANCE = new MongoHandlerRaw();

    public static MongoHandlerRaw getInstance() {
        return MongoHandlerRaw.INSTANCE;
    }
    ...
    private MongoHandlerRaw() {
        // opens the DB client for the thread of our executor
        executor.execute(() -> {
            mongoClient = new MongoClient(MongoHandlerRaw.DB_HOST, MongoHandlerRaw.DB_PORT);
            MongoCollection<Document> collection =
                    mongoClient.getDatabase(MongoHandlerRaw.DB_NAME).getCollection("names");
    ...
    public void handleNewValueInThread(String key, long latency) {
        ...
        executor.execute(() -> MongoHandlerRaw.handleNewValue(key, latency));
    }
    ...
    private static void handleNewValue(String key, long latency) {
        ...
        MongoHandlerRaw mH = MongoHandlerRaw.getInstance();
        MongoDatabase db = mH.mongoClient.getDatabase(MongoHandlerRaw.DB_NAME);
